...
from models import Autoencoder, AESegmentation
from trainer import SegmentationTrainer
from utils import plot_training_history, save_predictions, evaluate_and_save_metrics


def parse_args():
    parser = argparse.ArgumentParser(description='Train Segmentation with Pre-trained Encoder')
...
def main():
    ...
    save_dir = os.path.join(args.save_dir, f'ae_segmentation_{timestamp}')
    ...
    pretrained_ae = Autoencoder().to(device)
    checkpoint = torch.load(args.pretrained_path, map_location=device)
    pretrained_ae.load_state_dict(checkpoint['model_state_dict'])

    # Get encoder and create segmentation model
    encoder = pretrained_ae.get_encoder()
    model = AESegmentation(pretrained_encoder=encoder, n_classes=3).to(device)

    # Setup training
    criterion = nn.CrossEntropyLoss()
    # Only optimize decoder parameters since encoder is frozen
